...
class TestPrerequisiteEnforcement:
    ...
    async def test_no_prerequisites_means_available(
        ...
        a = await _make_node(db_session, learning_map, household, "A")
        met = await check_prerequisites_met(db_session, child.id, household.id, learning_map.id, a.id)
        assert met is True
    ...
    async def test_unmastered_prereq_blocks(
        ...
        met = await check_prerequisites_met(db_session, child.id, household.id, learning_map.id, b.id)
        assert met is False
    ...
    async def test_compute_map_state_blocked_and_available(
        ...
        states = await compute_map_state(db_session, child.id, household.id, learning_map.id)
...
class TestNodeAPI:
    ...
    async def test_create_update_delete_node(self, auth_client, db_session, household, subject):
        ...
        resp4 = await auth_client.delete(f"/api/v1/learning-maps/{map_id}/nodes/{node_id}")
...
class TestEdgeAPI:
    ...
    async def test_create_edge(self, auth_client, db_session, household, subject):
        ...
        resp2 = await auth_client.post(
            f"/api/v1/learning-maps/{map_id}/edges",
            json={
                "from_node_id": a_id,
                "to_node_id": b_id,
            },
    ...
    async def test_cycle_detection_409(self, auth_client, db_session, household, subject):
        ...
        await auth_client.post(
            f"/api/v1/learning-maps/{map_id}/edges",
            json={
                "from_node_id": nodes["B"],
                "to_node_id": nodes["C"],
            },
        )
        # C -> A should fail with 409
        resp_cycle = await auth_client.post(
            f"/api/v1/learning-maps/{map_id}/edges",
            json={
                "from_node_id": nodes["C"],
                "to_node_id": nodes["A"],
            },
        )
    ...
    async def test_delete_edge(self, auth_client, db_session, household, subject):
        ...
        edge_resp = await auth_client.post(
            f"/api/v1/learning-maps/{map_id}/edges",
            json={
                "from_node_id": r1.json()["id"],
                "to_node_id": r2.json()["id"],
        ...
        del_resp = await auth_client.delete(f"/api/v1/learning-maps/{map_id}/edges/{edge_id}")
...
class TestChildMapState:
    ...
    async def test_enrollment_and_map_state(
        ...
        # Get map state
        state_resp = await auth_client.get(f"/api/v1/children/{child.id}/map-state/{map_id}")
...
class TestParentOverride:
    ...
    async def test_override_creates_governance_event(
        ...
        gov_result = await db_session.execute(
            select(GovernanceEvent).where(GovernanceEvent.id == uuid.UUID(data["governance_event_id"]))
        )
...
class TestTemplates:
    ...
    async def test_copy_template_creates_independent_copy(
        ...
        resp = await auth_client.post("/api/v1/learning-maps/from-template/elementary-core")
        assert resp.status_code == 201
        data = resp.json()
        ...
        closure_result = await db_session.execute(
            select(LearningMapClosure).where(LearningMapClosure.learning_map_id == uuid.UUID(map_id))
        )
    ...
    async def test_template_not_found(self, auth_client):
        resp = await auth_client.post("/api/v1/learning-maps/from-template/nonexistent")
    ...
    async def test_two_copies_are_independent(self, auth_client, db_session, household):
        ...
        r1 = await auth_client.post("/api/v1/learning-maps/from-template/elementary-core")
        r2 = await auth_client.post("/api/v1/learning-maps/from-template/elementary-core")
