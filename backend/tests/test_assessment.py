...
class TestParentAssessment:
    ...
    async def test_assessment_overrides_mastery(
        self, auth_client, db_session, household, child, subject, learning_map, user,
    ):
        node = LearningNode(
            learning_map_id=learning_map.id, household_id=household.id,
            node_type=NodeType.skill, title="Assessed Node",
        )
        db_session.add(node)
        await db_session.flush()

        resp = await auth_client.post(f"/api/v1/children/{child.id}/assessments", json={
            "node_id": str(node.id),
    ...
    async def test_governance_event_on_override(
        ...
        node = LearningNode(
            learning_map_id=learning_map.id, household_id=household.id,
            node_type=NodeType.concept, title="Gov Event Node",
        )
        db_session.add(node)
        await db_session.flush()
        ...
        await auth_client.post(f"/api/v1/children/{child.id}/assessments", json={
            "node_id": str(node.id),
    ...
    async def test_confidence_override_triggers_fsrs(
        ...
        node = LearningNode(
            learning_map_id=learning_map.id, household_id=household.id,
            node_type=NodeType.skill, title="FSRS Override Node",
        )
        db_session.add(node)
        await db_session.flush()
...
class TestTranscript:
    ...
    async def test_transcript_maps_mastery_to_grades(
        ...
        for title, mastery in [("A Node", MasteryLevel.mastered), ("B Node", MasteryLevel.proficient)]:
            ...
            db_session.add(ChildNodeState(
                child_id=child.id, household_id=household.id,
                node_id=node.id, mastery_level=mastery,
...
class TestPortfolio:
    ...
    async def test_portfolio_crud(self, auth_client, db_session, household, child, user):
        resp = await auth_client.post(f"/api/v1/children/{child.id}/portfolio", json={
    ...
    async def test_portfolio_export(self, auth_client, db_session, household, child, user):
        ...
        db_session.add(PortfolioEntry(
            household_id=household.id, child_id=child.id,
            entry_type="narrative", title="Weekly Summary",
            subject="General", date_completed=date.today(),
        ))
        await db_session.flush()
