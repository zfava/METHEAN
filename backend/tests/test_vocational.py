    ...
    async def test_all_node_types_accepted(self, auth_client, db_session, household, user, subject, learning_map):
        ...
        for nt in [
            ...
            resp = await auth_client.post(
                f"/api/v1/learning-maps/{learning_map.id}/nodes",
                json={
                    "node_type": nt,
                    "title": f"Test {nt}",
...
class TestCertificationTracking:
    ...
    async def test_certification_crud(self, auth_client, db_session, household, child):
        ...
        resp = await auth_client.post(
            f"/api/v1/children/{child.id}/certifications",
            json={
        ...
        resp = await auth_client.put(
            f"/api/v1/children/{child.id}/certifications/{cert_id}",
            json={
                "status": "in_progress",
                "requirements": [
                    {"name": "Shop Safety", "met": True, "date_met": "2026-09-15"},
                    {"name": "Flat Position", "met": False},
                ],
            },
        )
        assert resp.status_code == 200
...
class TestMentors:
    ...
    async def test_mentor_crud(self, auth_client):
        ...
        resp = await auth_client.post(
            "/api/v1/household/mentors",
            json={
                "name": "Mike Johnson",
                "trade": "Welding",
                "availability": "Saturdays 9-12",
            },
        )
        assert resp.status_code == 201
