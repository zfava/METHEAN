...
async def _get_household_id(db: AsyncSession, child_id: str) -> uuid.UUID:
    ...
    result = await db.execute(select(Child.household_id).where(Child.id == uuid.UUID(child_id)))
...
async def test_golden_workflow(client: AsyncClient):
    ...
    plan = await client.post(
        f"/api/v1/children/{child_id}/plans/generate",
        json={
    ...
    for act in activities:
        approve = await client.put(f"/api/v1/plans/{plan_id}/activities/{act['id']}/approve")
        assert approve.status_code == 200, f"Step 11 APPROVE {act['id']} failed: {approve.text}"
