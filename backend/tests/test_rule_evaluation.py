...
from app.models.enums import (
    ActivityType, AttemptStatus, GovernanceAction,
    RuleScope, RuleTier, RuleType,
)
...
from app.services.governance import (
    ActivityContext, GovernanceDecision, evaluate_activity,
)
...
class TestPaceLimit:
    ...
    async def test_blocks_when_exceeded_hard(self, db_session, household, child, user):
        ...
        db_session.add(GovernanceRule(
            ...
            rule_type=RuleType.pace_limit, scope=RuleScope.household,
            name="60m limit", parameters={"max_daily_minutes": 60, "enforce": "hard"},
        ))
        ...
        week = PlanWeek(plan_id=plan.id, household_id=household.id, week_number=1, start_date=date.today(), end_date=date.today())
        db_session.add(week)
        await db_session.flush()
        act = Activity(plan_week_id=week.id, household_id=household.id, activity_type=ActivityType.lesson, title="T", governance_approved=True)
        db_session.add(act)
        await db_session.flush()
        db_session.add(Attempt(
            activity_id=act.id, household_id=household.id, child_id=child.id,
            status=AttemptStatus.completed, duration_minutes=50,
    ...
    async def test_warns_when_exceeded_soft(self, db_session, household, child, user):
        ...
        db_session.add(GovernanceRule(
            household_id=household.id, created_by=user.id,
            rule_type=RuleType.pace_limit, scope=RuleScope.household,
            name="60m soft", parameters={"max_daily_minutes": 60, "enforce": "soft"},
        ))
        plan = Plan(household_id=household.id, child_id=child.id, created_by=user.id, name="T", status=PlanStatus.active)
        db_session.add(plan)
        await db_session.flush()
...
class TestContentFilter:
    ...
    async def test_excludes_matching_topic(self, db_session, household, user):
        ...
        db_session.add(GovernanceRule(
            household_id=household.id, created_by=user.id,
            rule_type=RuleType.content_filter, scope=RuleScope.household,
            name="No evolution", parameters={"topic": "evolution", "stance": "exclude"},
        ))
        await db_session.flush()
...
class TestScheduleConstraint:
    ...
    async def test_blocks_weekend(self, db_session, household, user):
        ...
        db_session.add(GovernanceRule(
            household_id=household.id, created_by=user.id,
            rule_type=RuleType.schedule_constraint, scope=RuleScope.household,
            name="Weekdays only", parameters={
...
class TestAIBoundary:
    ...
    async def test_blocks_disallowed_role(self, db_session, household, user):
        ...
        db_session.add(GovernanceRule(
            ...
            name="Limited roles", parameters={"allowed_roles": ["planner", "tutor"]},
        ))
        await db_session.flush()
...
class TestConstitutionalViolation:
    ...
    async def test_always_blocks(self, db_session, household, user):
        ...
        db_session.add(GovernanceRule(
            ...
            name="Auto approve", parameters={"max_difficulty": 10, "action": "auto_approve"},
        ))
        # Add a constitutional ai_boundary that blocks
        db_session.add(GovernanceRule(
            household_id=household.id, created_by=user.id,
            rule_type=RuleType.ai_boundary, tier=RuleTier.constitutional,
...
class TestAggregateDecision:
    ...
    async def test_hierarchy_block_beats_review(self, db_session, household, user):
        ...
        db_session.add(GovernanceRule(
            household_id=household.id, created_by=user.id,
            rule_type=RuleType.approval_required, scope=RuleScope.household,
            name="Review", parameters={"min_difficulty": 1, "action": "require_review"},
        ))
        db_session.add(GovernanceRule(
            household_id=household.id, created_by=user.id,
            rule_type=RuleType.content_filter, scope=RuleScope.household,
            name="Block topic", parameters={"topic": "violence", "stance": "exclude"},
        ))
        await db_session.flush()
    ...
    async def test_all_rules_evaluated(self, db_session, household, user):
        ...
        for rt in [RuleType.approval_required, RuleType.pace_limit,
                   RuleType.content_filter, RuleType.schedule_constraint, RuleType.ai_boundary]:
            params = {"max_difficulty": 5, "action": "auto_approve"} if rt == RuleType.approval_required else \
                     {"max_daily_minutes": 999} if rt == RuleType.pace_limit else \
                     {"topic": "nothing_matches", "stance": "exclude"} if rt == RuleType.content_filter else \
                     {"allowed_days": ["monday","tuesday","wednesday","thursday","friday","saturday","sunday"]} if rt == RuleType.schedule_constraint else \
                     {"ai_transparency": "full"}
            db_session.add(GovernanceRule(
                household_id=household.id, created_by=user.id,
                rule_type=rt, scope=RuleScope.household,
                name=f"Test {rt.value}", parameters=params,
            ))
        await db_session.flush()
...
class TestBackwardCompat:
    ...
    async def test_old_style_call_works(self, db_session, household, user):
        ...
        db_session.add(GovernanceRule(
            household_id=household.id, created_by=user.id,
            rule_type=RuleType.approval_required, scope=RuleScope.household,
            name="Review hard", parameters={"min_difficulty": 3, "action": "require_review"},
        ))
        await db_session.flush()
