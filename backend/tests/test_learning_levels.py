...
from app.core.learning_levels import (
    LEARNING_LEVELS, VALID_LEVELS, SUBJECT_CATALOG,
    get_level_for_subject, get_daily_minutes_for_child, build_level_context,
)
...
class TestSubjectCatalogAPI:
    ...
    async def test_custom_subject_creation(self, auth_client):
        resp = await auth_client.post("/api/v1/subjects/custom", json={
            "name": "Beekeeping", "category": "life_skills", "description": "Hive management",
        })
...
class TestPreferencesAPI:
    ...
    async def test_create_child_without_grade(self, auth_client):
        resp = await auth_client.post("/api/v1/children", json={
            "first_name": "Test",
        })
        assert resp.status_code == 201
