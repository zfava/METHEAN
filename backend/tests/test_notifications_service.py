...
async def notif_user(db_session, notif_household):
    u = User(household_id=notif_household.id, email="notify@test.com",
...
class TestNotifications:
    ...
    async def test_mark_read(self, db_session, notif_household, notif_user):
        n = await send_notification(db_session, notif_household.id, notif_user.id,
                                    event_type="test", title="Read Test", body="B")
        await db_session.flush()
        if n:
    ...
    async def test_mark_all_read(self, db_session, notif_household, notif_user):
        await send_notification(db_session, notif_household.id, notif_user.id,
                                event_type="test", title="R1", body="B")
        await send_notification(db_session, notif_household.id, notif_user.id,
                                event_type="test2", title="R2", body="B")
        await db_session.flush()
