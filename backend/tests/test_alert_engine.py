...
async def alert_node(db_session, alert_household):
    ...
    await db_session.flush()
    n = LearningNode(
        learning_map_id=m.id, household_id=alert_household.id, node_type=NodeType.concept, title="Addition"
    )
    db_session.add(n)
    await db_session.flush()
    return n
...
class TestAlertEngine:
    async def test_stall_detection(self, db_session, alert_household, alert_child, alert_node):
        ...
        db_session.add(
            ChildNodeState(
                child_id=alert_child.id,
                household_id=alert_household.id,
                node_id=alert_node.id,
