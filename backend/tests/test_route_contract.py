...
class TestRouteContract:
    ...
    def test_protected_routes_exist(self):
        ...
        assert not missing, f"Routes in PROTECTED_ROUTES but not registered in app:\n" + "\n".join(
            f"  {m} {p}" for m, p in missing
        )
        assert not wrong_method, f"Routes with wrong method:\n" + "\n".join(
    ...
    def test_public_routes_exist(self):
        ...
        assert not missing, f"Routes in PUBLIC_ROUTES but not registered in app:\n" + "\n".join(
            f"  {m} {p}" for m, p in missing
        )
