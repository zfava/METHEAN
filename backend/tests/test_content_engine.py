...
class TestContentEnrichment:
    ...
    async def test_enrich_populates_content(
        ...
        resp = await auth_client.post(f"/api/v1/learning-maps/{learning_map.id}/nodes/{node.id}/enrich")
    ...
    async def test_enriched_content_has_required_fields(
        ...
        node = LearningNode(
            ...
            household_id=household.id,
            node_type=NodeType.skill,
            title="Enrichable Node",
            content={},
        )
        db_session.add(node)
        await db_session.flush()

        await auth_client.post(f"/api/v1/learning-maps/{learning_map.id}/nodes/{node.id}/enrich")

        resp = await auth_client.get(f"/api/v1/learning-maps/{learning_map.id}/nodes/{node.id}/content")
        assert resp.status_code == 200
        content = resp.json()
