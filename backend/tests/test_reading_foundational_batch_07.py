...
def test_counts_now_one_hundred_thirty():
    # Lower bound: rf-130 landed in this batch; later batches (rf-131+) raise these.
...
def test_library_ids_are_contiguous_no_gaps_no_duplicates():
    ...
    assert top >= 130, f"library top should be at least rf-130, got rf-{top}"
...
async def test_generator_plan_reading_tier_zero_needs_content(db_session, household):
    ...
    # Lower bound: rf-01..rf-130 all resolve; later batches add more resolvable refs.
