...
@pytest.mark.asyncio
async def test_owner_can_read_any_child_in_household(
    auth_client: AsyncClient, child: Child, second_child: Child
):
...
@pytest.mark.asyncio
async def test_owner_can_write_any_child_in_household(
    auth_client: AsyncClient, child: Child, second_child: Child
):
...
@pytest.mark.asyncio
async def test_co_parent_can_read_any_child(
    co_parent_client: AsyncClient, child: Child, second_child: Child
):
...
@pytest.mark.asyncio
async def test_co_parent_can_write_any_child(
    co_parent_client: AsyncClient, child: Child
):
...
@pytest.mark.asyncio
async def test_observer_can_read_any_child(
    observer_client: AsyncClient, child: Child
):
...
@pytest.mark.asyncio
async def test_observer_cannot_write_any_child(
    observer_client: AsyncClient, child: Child
):
...
@pytest.mark.asyncio
async def test_self_learner_can_access_own_child(
    self_learner_client: AsyncClient, child: Child
):
...
@pytest.mark.asyncio
async def test_self_learner_cannot_access_other_child(
    self_learner_client: AsyncClient, second_child: Child
):
    r = await self_learner_client.get(
        f"/api/v1/children/{second_child.id}/dashboard"
    )
    assert r.status_code == 403, r.text
...
async def test_list_children_filters_to_linked_child_for_self_learner(
    ...
    assert str(second_child.id) not in ids, (
        "self-learner must not see siblings via /children"
    )
...
async def other_household_user(
...
@pytest.mark.asyncio
async def test_user_from_other_household_gets_404(
    other_household_user: tuple[User, AsyncClient], child: Child
):
...
@pytest.mark.asyncio
async def test_dependency_returns_child_object_to_route_handler(
    auth_client: AsyncClient, child: Child
):
