...
class FamilyInsight(Base):
    ...
    status: Mapped[InsightStatus] = mapped_column(
        nullable=False, default=InsightStatus.detected
    )
...
class FamilyInsightConfig(Base):
    ...
    __table_args__ = (
        UniqueConstraint("household_id", name="uq_insight_config_household"),
    )
    ...
    pattern_settings: Mapped[dict] = mapped_column(JSONB, default=lambda: {
