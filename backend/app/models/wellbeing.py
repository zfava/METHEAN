...
class WellbeingAnomaly(Base):
    ...
    __table_args__ = (
        ...
        Index(
            ...
            "child_id", "anomaly_type", "created_at",
            postgresql_using="btree",
        ),
    )
    ...
    status: Mapped[AnomalyStatus] = mapped_column(
        nullable=False, default=AnomalyStatus.detected
    )
...
class WellbeingConfig(Base):
    ...
    __table_args__ = (
        UniqueConstraint("child_id", name="uq_wellbeing_config_child"),
    )
    ...
    sensitivity_level: Mapped[SensitivityLevel] = mapped_column(
        nullable=False, default=SensitivityLevel.balanced
    )
