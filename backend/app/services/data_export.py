...
from app.models.evidence import ActivityFeedback, AdvisorReport, ReadingLogEntry, WeeklySnapshot
...
async def export_family_data(db: AsyncSession, household_id: uuid.UUID) -> bytes:
    ...
    hh = hh_r.scalar_one()
    zf.writestr("family_profile.json", json.dumps({
        "name": hh.name,
    ...
    rules = [
        {"name": r.name, "type": r.rule_type.value, "tier": r.tier.value,
         "scope": r.scope.value, "parameters": r.parameters, "is_active": r.is_active}
    ...
    events_r = await db.execute(
        select(GovernanceEvent).where(GovernanceEvent.household_id == household_id)
        .order_by(GovernanceEvent.created_at)
    )
    events = [
        {"action": e.action.value, "target_type": e.target_type,
         "target_id": str(e.target_id), "reason": e.reason, "created_at": str(e.created_at)}
    ...
    for child in children:
        ...
        curricula = [
            {"subject": c.subject_name, "year": c.academic_year, "grade": c.grade_level,
             "status": c.status, "scope_sequence": c.scope_sequence, "actual_record": c.actual_record}
        ...
        reading = [
            {"title": r.book_title, "author": r.book_author, "genre": r.genre,
             "status": r.status, "pages_read": r.pages_read, "pages_total": r.pages_total,
             "narration": r.narration, "rating": r.child_rating}
        ...
        states_r = await db.execute(
            select(ChildNodeState).where(ChildNodeState.child_id == child.id)
        )
        states = [
            {"node_id": str(s.node_id), "mastery_level": s.mastery_level.value if hasattr(s.mastery_level, "value") else str(s.mastery_level),
             "time_spent_minutes": s.time_spent_minutes, "attempts_count": s.attempts_count}
        ...
        snapshots = [
            {"week_start": str(s.week_start), "week_end": str(s.week_end),
             "total_minutes": s.total_minutes, "activities_completed": s.activities_completed,
