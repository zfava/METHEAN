...
TUTOR_PROFILE = RoleContextProfile(
    ...
    sources=[
        ...
        # Relationship memory: opt in derived milestones (the journey so
        # far). Placed right after the register and ahead of the profile
        # entries; required so its position is deterministic, but empty
        # (and therefore skipped) unless the parent turned it on.
        ContextSource("relationship_milestones", "fetch_milestones", 280, 0.9, 365, required=True),
...
async def fetch_milestones(db, child_id, household_id, **kw) -> dict:
    """Relationship memory milestone block for the tutor.

    Reads through services/learning_context.py so the role gate, the opt
    in preference gate, and the derived (never stored) milestones live in
    one place. Only registered on the tutor profile, so it never fires for
    another role.
    """
    from app.services.learning_context import build_milestone_block

    text = await build_milestone_block(db, household_id, child_id, role="tutor")
    return {"text": text, "metadata": {"timestamp": _now()}}
