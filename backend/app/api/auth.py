...
async def register(
    ...
    # Issue + send the email verification token. Email send is best-effort —
    # registration must not 500 if SMTP is degraded.
    ...
    try:
        await send_email(user.email, "Verify your METHEAN email", email_verification_email(verify_url))
    except Exception:
        pass
...
async def verify_email(
    ...
    """Validate a hashed, expiring, single-use verification token."""
...
async def resend_verification(
    ...
    """Issue a fresh single-use verification token and send a new email."""
    from app.core.config import settings
    from app.services.email import send_email
    from app.services.email_templates import email_verification_email
    from app.services.email_verification import issue_token
