...
async def list_children(
    ...
    result = await db.execute(
        select(Child)
        .options(selectinload(Child.preferences))
        .where(
    ...
    if hasattr(body, "subject_levels") and body.subject_levels is not None:
        ...
        for subj, level in body.subject_levels.items():
            if level not in VALID_LEVELS:
                raise HTTPException(
                    400, f"Invalid level '{level}' for '{subj}'. Must be one of: {', '.join(sorted(VALID_LEVELS))}"
                )
...
async def add_mentor(
    body: MentorCreate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
) -> dict:
    household = (await db.execute(select(Household).where(Household.id == user.household_id))).scalar_one()
    settings = dict(household.settings or {})
...
async def remove_mentor(
    mentor_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
) -> dict:
    household = (await db.execute(select(Household).where(Household.id == user.household_id))).scalar_one()
    settings = dict(household.settings or {})
    mentors = list(settings.get("mentors", []))
