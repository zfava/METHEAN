...
async def _enrich_map(
    ...
    When ``session_factory`` is provided it is used as-is (the caller owns the
    engine lifecycle); otherwise an engine is created from settings and disposed
    before returning. This mirrors ``capture_weekly_snapshots`` and lets tests
    drive the task against the test database.
    """
