...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("predictive_child_id", sa.dialects.postgresql.UUID(as_uuid=True),
                   sa.ForeignKey("children.id", ondelete="SET NULL"), nullable=True),
        sa.Column("predictive_node_id", sa.dialects.postgresql.UUID(as_uuid=True),
                   sa.ForeignKey("learning_nodes.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("pattern_settings", postgresql.JSONB, nullable=False,
                   server_default=default_settings),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
