...
""")
    ...
    op.execute(f"DROP POLICY IF EXISTS {NULLABLE_HID_TABLE}_household_isolation ON {NULLABLE_HID_TABLE}")
    ...
    op.execute(f"CREATE POLICY {NULLABLE_HID_TABLE}_household_isolation ON {NULLABLE_HID_TABLE} {SAFE_USING_NULLABLE}")
...
def downgrade() -> None:
    ...
    old_using = "USING (household_id = current_setting('app.current_household_id')::uuid)"
    old_using_nullable = (
        "USING (household_id = current_setting('app.current_household_id')::uuid OR household_id IS NULL)"
    )
    ...
    op.execute(f"DROP POLICY IF EXISTS {NULLABLE_HID_TABLE}_household_isolation ON {NULLABLE_HID_TABLE}")
    op.execute(f"CREATE POLICY {NULLABLE_HID_TABLE}_household_isolation ON {NULLABLE_HID_TABLE} {old_using_nullable}")
