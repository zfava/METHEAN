...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_wellbeing_child_status_created ON wellbeing_anomalies (child_id, status, created_at DESC)"
    )
    op.execute("CREATE INDEX IF NOT EXISTS ix_wellbeing_household_status ON wellbeing_anomalies (household_id, status)")
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_wellbeing_child_type_created ON wellbeing_anomalies (child_id, anomaly_type, created_at DESC)"
    )
