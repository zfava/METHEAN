...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_beta_feedback_household_created "
        "ON beta_feedback (household_id, created_at DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_beta_feedback_user_created "
