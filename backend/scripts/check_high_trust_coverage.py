...
The global coverage floor (50%) is a project-wide signal, but the
modules that gate authentication, payments, and authorization need a
much tighter bar. This script reads ``coverage.json`` (which
``pytest --cov-report=json`` produces) and exits non-zero when any
high-trust file is below the floor.

Modules listed but absent from the report (because the branch hasn't
shipped them yet, or they were removed) emit a warning rather than
failing — the call site is the place to track the gap.
"""
